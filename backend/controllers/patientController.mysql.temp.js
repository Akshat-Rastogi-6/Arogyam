...
export const getPatientDetails = async (req, res) => {
  try {
    ...
    // Fetch health info
    const healthInfo = (
      await pool.query("SELECT * FROM patient_info WHERE patient_id = ?", [req.patientId])
    ).rows[0] || {};
...
export const updatePatientDetails = async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT * FROM patients WHERE id = ?', [req.patientId]);
    ...
    if (healthResult.rows.length > 0) {
      ...
      await pool.query(
        ...
        SET age = ?, gender_identity = ?, height = ?, weight = ?, blood_type = ?,
            ...
            drug_details = ?, exercise_frequency = ?0, diet_description = ?1, sleep_hours = ?2,
            chronic_conditions = ?3, medications = ?4, allergies = ?5, surgeries = ?6, family_history = ?7,
            mental_health_conditions = ?8, last_checkup = ?9, vaccinations_up_to_date = ?0, dental_checkups = ?1,
            sexual_performance_issues = ?2, libido_concerns = ?3, testicular_pain_lumps = ?4, urination_issues = ?5,
            prostate_exam = ?6, weight_changes = ?7, hair_loss_concerns = ?8, menstrual_start_age = ?9,
            menstrual_regular = ?0, severe_cramps = ?1, heavy_bleeding = ?2, pregnancy_status = ?3,
            pregnancy_count = ?4, pregnancy_complications = ?5, menopause_symptoms = ?6, menopause_start_age = ?7,
            breast_self_exam = ?8, last_mammogram = ?9, breast_changes = ?0, last_pap_smear = ?1,
            gynecological_conditions = ?2
        WHERE patient_id = ?3`,
        [
